...
(* CHANGE: Add a case for Shift expressions *)
      | Shift(dx,dy,e1) =>
          case eval_prog (e1,env) of
             ...
             | _ => raise Impossible "Unable to Shift unknown geom_exp"
...
fun preprocess_prog e =
	...
	NoPoints => e (* first 5 cases are all values, so no computation *)
      ...
      | LineSegment(x1,y1,x2,y2) =>
        ...
        then 
          if real_close(y1,y2)
          then Point(x1,y1) (* start and end are real close, return point *)
          else 
            if y1 < y2 then e else LineSegment(x2,y2,x1,y1) (*flip line if y1 is higher *)
        else (* check that x1 is west of x2 *)
          if x1 < x2 then e else LineSegment(x2,y2,x1,y1)
