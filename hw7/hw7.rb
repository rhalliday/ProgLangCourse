...
class LineSegment < GeometryValue
  ...
  def preprocess_prog
    if real_close(x1,x2) 
      if real_close(y1,y2)
        Point.new(x1,y1) # start and end are real close, return point
      else
        if y1 < y2
          self
        else 
          LineSegment.new(x2,y2,x1,y1) # flip line if y1 is higher
        end
      end
    else # check that x1 is west of x2
      if x1 < x2 
        self
      else 
  ...
  def intersectWithSegmentAsLineResult seg
    # set up segment a and b (we can flip them if we have to)
    ...
    if real_close(seg.x1,seg.x2)
      if seg.y1 < y1 # let segment a start at or below start of segment b
        aXstart,aYstart,aXend,aYend = seg.x2,seg.y1,seg.x2,seg.y2
      ...
      if seg.x1 < x1 # let segment a start at or to the left of start of segment b
...
class Var < GeometryExpression
  ...
  def eval_prog env 
    myelement = env.find { |i| i[0] == @s }
    if myelement
      myelement[1].eval_prog(env)
    else
